...
// Apps Script deployment URL keessa (Web App URL) as galchi (guidance keessatti argama)
const API_URL = 'https://script.google.com/macros/s/AKfycbx5-SpP3IqsX4Q3jBaNfXCZqlLzVtHQDyiB6zioersVqxKCmV-iRx5IyxE_9un7t8p6nw/exec';
...
async function loadData() {
  try {
    ...
    if (data.courses && data.courses.length) {
      ...
      await apiPost('saveCourses', { courses }); // seed the sheet on first run
    }
    if (data.settings && Object.keys(data.settings).length) {
      settings = data.settings;
    } else {
      settings = { ...DEFAULT_SETTINGS };
      await apiPost('saveSettings', { settings }); // seed the sheet on first run
    }
    results = data.results || [];
    attempted = data.attempted || [];
  } catch (err) {
    console.error('Google Sheets-tti walqabachuu hin dandeenye, daataa lafa qofa fayyadamaa jira:', err);
    courses = JSON.parse(JSON.stringify(DEFAULT_COURSES));
    settings = { ...DEFAULT_SETTINGS };
    results = [];
    attempted = [];
...
async function goStudentRegister() {
  await loadData();
...
async function saveCourses() {
  try {
    await apiPost('saveCourses', { courses });
  } catch (err) {
    alert("Google Sheets-tti olkaa'uu hin dandeenye. Interneetii kee mirkaneeffadhu.\n" + err);
  }
}
...
async function submitExam() {
  ...
  const attemptedKey = currentStudent.id + '::' + currentStudent.courseId;
  const result = {
  ...
  attempted.push(attemptedKey);
  results.push(result);
  showResults(correct, total, pct, passed);
  try {
    await apiPost('addResult', { result, attemptedKey });
  } catch (err) {
    console.error('Bu\'aan Google Sheets-tti galfamuu hin dandeenye:', err);
  }
}
...
async function switchTab(tabId, btn) {
  ...
  if (tabId === 'tab-results')  { await refreshResults(); loadResultsTab(); }
  if (tabId === 'tab-courses')  { backToCourses(); renderCoursesGrid(); }
  if (tabId === 'tab-settings') loadSettingsUI();
}

// Pulls the latest results from Google Sheets (so results submitted by
// students on other devices show up here too).
async function refreshResults() {
  try {
    ...
    console.error('Bu\'aa haaraa fiduu hin dandeenye:', err);
  }
}
...
async function clearAllData() {
  try {
    await apiPost('clearResults', {});
  } catch (err) {
    alert('Google Sheets irraa haqu hin dandeenye.\n' + err);
  }
  results = [];
  attempted = [];
...
async function saveSettings() {
  ...
  try {
    ...
    msg.textContent = '❌ Failed to save to Google Sheets. Check your connection.';
  }
  // API key is a personal credential — kept local to this device only, not shared.
...
// ===== KEYBOARD SHORTCUTS =====
// Preload data from Google Sheets in the background so the first
// screen (student register / admin login) opens with data ready.
loadData();
...
async function extractFromPdf() {
  ...
  if (!file) { showPdfError('Duraan PDF file filadhu!'); return; }
  ...
  document.getElementById('pdf-save-btn').style.display = 'none';
  setPdfProgress(10, '📖 PDF dubbisaa jira...');
  ...
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      setPdfProgress(10 + Math.round((i / pdf.numPages) * 40), `📖 Fuula ${i}/${pdf.numPages} dubbisaa jira...`);
    }

    if (fullText.trim().length < 50) {
      showPdfError('PDF keessaa barreeffama argachuu hin dandeenye. Scanned/image PDF ta\'uu mala.');
      resetPdfBtn(); return;
    }

    // Step 2: Parse questions with regex (no API needed)
    setPdfProgress(60, '🔍 Gaaffilee barbaadaa jira...');
    pdfExtractedQuestions = parseQuestionsFromText(fullText);
    setPdfProgress(100, '✅ Xumurameera!');

    if (pdfExtractedQuestions.length === 0) {
      showPdfError('Gaaffii argachuu hin dandeenye. PDF format: "1. Question\\nA. ...\\nB. ...\\nC. ...\\nD. ..." ta\'uu qaba.');
      resetPdfBtn(); return;
    }
    ...
    showPdfError('Dogoggora: ' + err.message);
    resetPdfBtn();
  }
}
...
function parseQuestionsFromText(text) {
  ...
  while (i < lines.length) {
    ...
    if (isTF) {
      questions.push({ q: cleanQText(qText), opts: ['True', 'False'], ans: -1 });
...
function showPdfPreview(questions) {
  ...
  list.innerHTML = questions.map((q, i) => {
    ...
    return `<div id="pdf-qblock-${i}" style="margin-bottom:0.9rem;padding:0.7rem;background:#f0f0f0;border:1px solid #d3d3d3;border-radius:8px">
      ...
      <div id="pdf-ans-label-${i}" style="font-size:0.72rem;color:gray;margin-top:4px">⬜ Deebii hin filatamne</div>
    </div>`;
  }).join('');
...
function selectPdfAns(qIdx, ansIdx) {
  ...
  if (lbl) { lbl.textContent = `✅ Deebii: ${label}`; lbl.style.color = 'darkgreen'; }
...
async function savePdfQuestions() {
  ...
  await saveCourses();
  renderQuestions(activeCourseId);

  showPdfSuccess(`🎉 Gaaffilee ${pdfExtractedQuestions.length} course-tti galchamani!`);
  document.getElementById('pdf-save-btn').style.display = 'none';
  document.getElementById('pdf-preview-area').style.display = 'none';
  pdfExtractedQuestions = [];
