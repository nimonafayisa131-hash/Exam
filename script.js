...
async function loadData() {
  try {
    ...
    if (data && data.courses && data.courses.length) {
      ...
      await fbPut('courses', courses); // seed on first run
    }
    if (data && data.settings && Object.keys(data.settings).length) {
      settings = data.settings;
    } else {
      settings = { ...DEFAULT_SETTINGS };
      await fbPut('settings', settings); // seed on first run
    }
    results = data && data.results ? Object.values(data.results) : [];
    attempted = data && data.attempted ? Object.keys(data.attempted) : [];
  } catch (err) {
    console.error('Firebase-tti walqabachuu hin dandeenye, daataa lafa qofa fayyadamaa jira:', err);
    courses = JSON.parse(JSON.stringify(DEFAULT_COURSES));
    settings = { ...DEFAULT_SETTINGS };
    results = [];
    attempted = [];
...
async function saveCourses() {
  try {
    await fbPut('courses', courses);
  } catch (err) {
    alert("Firebase-tti olkaa'uu hin dandeenye. Interneetii kee mirkaneeffadhu.\n" + err);
  }
}
...
async function submitExam() {
  ...
  try {
    await fbPost('results', result);
    await fbPut(`attempted/${fbKey(attemptedKey)}`, true);
  } catch (err) {
    console.error("Bu'aan Firebase-tti galfamuu hin dandeenye:", err);
  }
}
...
async function clearAllData() {
  try {
    await fbPut('results', null);
    await fbPut('attempted', null);
  } catch (err) {
    alert("Firebase irraa haqu hin dandeenye.\n" + err);
  }
  results = [];
  attempted = [];
